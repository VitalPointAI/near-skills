 ...
 */
export async function near_indexer_query(params: {
  account: string;
  ...
  try {
    ...
    switch (query_type) {
      case 'ft_transfers': {
        const response = await getAccountFTTransactions(account, {
      ...
      case 'transactions':
      default: {
        const response = await getAccountTransactions(account, {
          per_page: Math.min(limit, 25),
          order,
        });
        ...
        for (const tx of response.txns.slice(0, limit)) {
          ...
          if (tx.actions && tx.actions.length > 0) {
            const actions = tx.actions.map(a => {
              if (a.method) return `${a.action}:${a.method}`;
              return a.action;
            }).join(', ');
            result += `   Actions: ${actions}\n`;
          }
          
          const deposit = formatNear(tx.actions_agg?.deposit || 0);
          if (parseFloat(deposit) > 0) {
            result += `   Amount: ${deposit} Ⓝ\n`;
          }
          
          const fee = formatNear(tx.outcomes_agg?.transaction_fee || 0);
          result += `   Fee: ${fee} Ⓝ\n`;
 ...
 */
export async function near_account_activity(params: {
  account: string;
  ...
  try {
    const response = await getAccountActivities(account);
    ...
    if (direction !== 'all') {
      activities = activities.filter(a => 
        a.direction.toLowerCase() === direction.toLowerCase()
      );
    }
    ...
    for (const activity of activities.slice(0, limit)) {
      const date = formatTimestamp(activity.block_timestamp).split('T')[0];
    ...
    for (const [date, dayActivities] of Object.entries(byDate)) {
      ...
      for (const activity of dayActivities) {
        const time = formatTimestamp(activity.block_timestamp).split('T')[1].slice(0, 5);
        const direction = activity.direction === 'INBOUND' ? '⬇️' : '⬆️';
        const balance = formatNear(activity.absolute_nonstaked_amount);
 ...
 */
export async function near_contract_calls(params: {
  account: string;
  contract?: string;
  method?: string;
  limit?: number;
  ...
  try {
    ...
    let functionCalls = response.txns.filter(tx =>
      tx.actions?.some(a => a.action === 'FUNCTION_CALL')
    );
    
    // Filter by contract if specified
    if (contract) {
      functionCalls = functionCalls.filter(tx =>
        tx.receiver_account_id === contract || tx.signer_account_id === contract
      );
    }
    ...
    if (contract) result += `Contract Filter: ${contract}\n`;
    if (method) result += `Method Filter: ${method}\n`;
    result += '\n';
    ...
    for (const tx of functionCalls.slice(0, limit)) {
      ...
      for (const call of funcCalls) {
        ...
        if (call.deposit && Number(call.deposit) > 0) {
          result += `      Deposit: ${formatNear(call.deposit)} Ⓝ\n`;
        }
      }
 ...
 */
export async function lookupTransaction(txHash: string): Promise<string> {
  if (!txHash) {
  ...
  try {
    ...
    const date = formatTimestamp(tx.block_timestamp);
    ...
    if (tx.actions && tx.actions.length > 0) {
      ...
      for (const action of tx.actions) {
        ...
        if (action.method) result += `: ${action.method}`;
        if (action.deposit && Number(action.deposit) > 0) {
          result += ` (${formatNear(action.deposit)} Ⓝ)`;
        }
        result += '\n';
      }
    }
    
    result += `\n**Fee:** ${formatNear(tx.outcomes_agg?.transaction_fee || 0)} Ⓝ\n`;
